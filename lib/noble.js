...
Noble.prototype.startScanning = function(serviceUuids, allowDuplicates, passive, callback) {
  var scan = function(state) {
	...
	this._bindings.startScanning(serviceUuids, allowDuplicates, passive);
    }
  };
...
Noble.prototype.onDiscover = function(uuid, address, addressType, connectable, advertisement, rssi) {
	...
	//this.emit('disover', peripheral)
	// Just use a different event for now
	this.emit('passive-scan', {uuid:uuid, advertisement: advertisement, rssi: rssi});
    } else {
